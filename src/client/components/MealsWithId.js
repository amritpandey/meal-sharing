...
export const MealsWithId = () => {
    ...
    useEffect(()=>{
    const result = fetch(`http://localhost:3000/api/meals?availableReservations`)
    .then(res=>res.json())
    ...
    return (
        <div>
            <section className="single-item-detail">
                ...
                {specificReview}
                {availableGuests >0? <p>Available seats : {availableGuests}</p>:<p>Available seats : 0</p>}
                <Link to="/meals"><p>Back</p></Link>
            </section>
            {availableGuests > 0 ? (
                ...
                <p style={{textAlign:"center", color:"red"}}>No seats available for this meal.</p>
            )}
        </div>
    );
};
